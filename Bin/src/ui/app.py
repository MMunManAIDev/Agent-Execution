...
class AgentExecutiveApp:
    ...
    # Main application initialization in run.py
    if __name__ == "__main__":
        logging.basicConfig(level=logging.INFO)
        root = tk.Tk()
        web_handler = WebPageHandler()  # Initialize properly before passing
        app = AgentExecutiveApp(root, web_handler)
        root.mainloop()
    
    def __init__(self, root: tk.Tk, web_handler: WebPageHandler):
        """
        Initialiseer de hoofdapplicatie.
        
        Args:
            root: De Tkinter root widget
            web_handler: De WebPageHandler voor browserinteracties
        """
        self.periodic_update_id = None
        self.root = root
        self.web_handler = web_handler  # Store the web handler for use throughout the application
        ...
        try:
            # Setup hoofdwindow
            self.logger.info("Setting up the main window")
            self.setup_window()
            ...
            # Setup UI
            self.logger.info("Setting up the UI components")
            self.setup_ui()
            ...
            self.logger.error(f"Failed during application initialization: {str(e)}")
    ...
    def setup_handlers(self) -> None:
        ...
        try:
            ...
            # Use the passed web_handler instead of creating a new instance here
    ...
    def start_periodic_updates(self) -> None:
        ...
        def update():
            ...
            # Schedule next update and store the ID
            self.periodic_update_id = self.root.after(1000, update)

        # Initial call to start the periodic update loop
        update()
    ...
    def on_close(self) -> None:
        ...
        if self.tab_manager.has_executing_tabs():
            if not messagebox.askyesno("Warning", 
                ...
                return
        try:
            # Cancel any pending periodic updates
            if self.periodic_update_id:
                self.root.after_cancel(self.periodic_update_id)

            # Use asyncio to schedule the cleanup coroutine properly
            asyncio.create_task(self.cleanup())

            # Destroy the root window (should be called after cleanup)
            self.root.after(100, self.root.destroy)

        except Exception as e:
            self.logger.error(f"Error during shutdown: {str(e)}")
            messagebox.showerror("Shutdown Error", f"Error during shutdown:\n{str(e)}")
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            if hasattr(self, 'web_handler'):
                await self.web_handler.cleanup()  # Cleanup asynchronously
