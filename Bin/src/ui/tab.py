...
class ExecutionTab(ttk.Frame):
    ...
    def setup_bindings(self):
        ...
        # URL bar bindings
        self.url_bar.entry.bind("<Return>", lambda e: asyncio.create_task(self.on_url_navigate(self.url_bar.get_url())))
        
    async def on_url_navigate(self, url: str) -> None:
        """Handle URL navigation event."""
        try:
            if not url:
                self.logger.warning("Navigation failed: No URL provided")
                return

            await self.app.web_handler.load_page(url)  # Make sure to await here for async call
        except Exception as e:
            self.app.handle_error(e, context="Navigating URL")
            
    def navigate_to_url(self, url: str):
        """Handles navigation triggered by the user."""
        asyncio.create_task(self.on_url_navigate(url))
